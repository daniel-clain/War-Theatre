...
import { ClientPlayerOptions } from "../../game/main-types/player";
...
export function useHostService(websockets: ReturnType<typeof useWebsockets>) {
  const thisClientId = getThisClientId();
  const [localName, setLocalName] = useState<string | undefined>(getThisName());
  const { connected, send, hostState, playerGameState } = websockets;
